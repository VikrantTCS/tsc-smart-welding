...
def getFDHUrl(config):
	if not hasattr(config,'FDH_URL') :
		...
		print (cfTarget)
		config.FDH_URL=cfTarget.split('urls:')[1].strip().split('last uploaded:')[0].strip()
...
def getAnalyticsUrl(config):
		...
		print (cfTarget)
		config.RMD_ANALYTICS_URL="https://"+cfTarget.split('urls:')[1].strip().split('last uploaded:')[0].strip()
...
def getRabbitMQConsumerUrl(config):
	if not hasattr(config,'RABBITMQ_CONSUMER_URL') :
		...
		print (cfTarget)
		config.RABBITMQ_CONSUMER_URL="https://"+cfTarget.split('urls:')[1].strip().split('last uploaded:')[0].strip()
...
def getOrchClientUrl(config):
	...
	print (cfTarget)
	config.RMD_ORCH_URL="https://"+cfTarget.split('urls:')[1].strip().split('last uploaded:')[0].strip()
