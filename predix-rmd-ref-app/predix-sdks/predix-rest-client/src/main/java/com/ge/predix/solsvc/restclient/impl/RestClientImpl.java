...
public class RestClientImpl implements RestClient, ApplicationContextAware {
	 ...
	 */
	private CloseableHttpClient getHttpClient(int connectionTimeout, int socketTimeout, String proxyHost, String proxyPort) {

		Builder requestBuilder = RequestConfig.custom();
		if (this.restConfig.getPoolConnectionRequestTimeout() > 0) {
			requestBuilder = requestBuilder.setConnectionRequestTimeout(this.restConfig.getPoolConnectionRequestTimeout()); //timeout to request from connection manager
		}
		if (connectionTimeout > 0) {
			requestBuilder = requestBuilder.setConnectTimeout(connectionTimeout); //timeout till connection with server is established
		}
		if (socketTimeout > 0) {
			requestBuilder = requestBuilder.setSocketTimeout(socketTimeout); //timeout to receive the data from the server
		}

		HttpClientBuilder builder = HttpClients.custom().setConnectionManager(this.poolManager).setConnectionManagerShared(true)
	...
	public String requestToken(List<Header> headers, String oauthResource, String oauthHost, String oauthPort,
		...
		try (CloseableHttpClient httpClient = getHttpClient(this.restConfig.getOauthConnectionTimeout(), this.restConfig.getOauthSocketTimeout(), proxyHost, proxyPort);){
			...
			if (!oauthResource2.startsWith("/"))
				oauthResource2 = "/" + oauthResource2;
	...
	private String performPost(String url, CloseableHttpClient httpClient, String requestBody, List<Header> headers2) throws ClientProtocolException, IOException
	{
	...
	private void setupSecureContext(String certLocation, String certPassword) {
		...
		try {
			...
			if (certLocation2 != null && !certLocation2.isEmpty() && certPassword != null && !certPassword.isEmpty()) {
				...
				//SSLConnectionSocketFactory sslsf = new SSLConnectionSocketFactory(localSSLContext,
	...
	public CloseableHttpResponse get(String url, List<Header> headers, int connectionTimeout, int socketTimeout) {		
		try (CloseableHttpClient httpClient = getHttpClient(connectionTimeout, socketTimeout, this.restConfig.getOauthProxyHost(),
	...
	@SuppressWarnings({ "nls" })
	@Override
	public CloseableHttpResponse post(String url, String request, List<Header> headers, int connectionTimeout, int socketTimeout) {
		try {
			HttpPost method = new HttpPost(url);
	...
	@SuppressWarnings({ "nls" })
	@Override
	public CloseableHttpResponse post(String url, HttpEntity entity, List<Header> headers, int connectionTimeout, int socketTimeout) {
		...
		try (CloseableHttpClient httpClient = getHttpClient(connectionTimeout, socketTimeout, this.restConfig.getOauthProxyHost(),
			...
			try {
				throw new RuntimeException(
						"unable to call url=" + url + " with headers=" + headers + " and body=" + EntityUtils.toString(entity), e);
	...
	@SuppressWarnings({ "nls" })
	@Override
	public CloseableHttpResponse put(String url, String request, List<Header> headers,int connectionTimeout, int socketTimeout) {
