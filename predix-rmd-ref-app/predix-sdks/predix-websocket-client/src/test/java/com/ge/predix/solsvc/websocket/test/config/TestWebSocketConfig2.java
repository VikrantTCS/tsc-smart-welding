 ...
 * 
 * @author 212438846
 * Class TestWebSocketConfig2 implements IWebSocketConfig. Every separate websocket server connection requires the
 * IWebSocketConfig interface to be implemented per connection.   
 */
@Component
public class TestWebSocketConfig2
        implements IWebSocketConfig
{

    @Value("${predix.oauth.proxyHost2:}")
    private String wsProxyHost;

    @Value("${predix.oauth.proxyPort2:}")
    private String wsProxyPort;

    @Value("${predix.websocket.uri2}")
    private String wsUri;

    @Value("${predix.websocket.zoneid2}")
    private String zoneId;

    @Value("${predix.websocket.pool.maxIdle2:}")
    private int    wsMaxIdle;

    @Value("${predix.websocket.pool.maxActive2:}")
    private int    wsMaxActive;

    @Value("${predix.websocket.pool.maxWait2}")
