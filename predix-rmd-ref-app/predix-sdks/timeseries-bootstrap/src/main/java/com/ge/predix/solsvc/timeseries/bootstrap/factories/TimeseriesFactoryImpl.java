...
public class TimeseriesFactoryImpl implements TimeseriesFactory {
	...
	public void createConnectionToTimeseriesWebsocket() {
		try {
			this.wsClient.init(this.tsIngestionWSConfig);
		} catch (Exception e) {
	...
	@Override
	public void createConnectionToTimeseriesWebsocket(
			WebSocketAdapter messageListener) {
		createConnectionToTimeseriesWebsocket();
		this.mListener = messageListener;
	}

	private void registerDefaultMessageListener() {
		this.mListener = new WebSocketAdapter() {
			...
			private void handleErrorMessage(String message) {
				...
				if (am.getStatusCode() > 299 && am.getStatusCode() < 400) {
					this.logger
							.info("STATUS CODE...." + am.getStatusCode() + "--ID:" + am.getMessageId()); //$NON-NLS-1$ //$NON-NLS-2$
				} else if (am.getStatusCode() > 399) {
					this.logger
							.error("ERROR STATUS CODE...." + am.getStatusCode() + "--ID:" + am.getMessageId()); //$NON-NLS-1$ //$NON-NLS-2$
				} else {
					this.logger
							.debug("SUCCESS...." + am.getStatusCode() + "--ID:" + am.getMessageId()); //$NON-NLS-1$ //$NON-NLS-2$
	 ...
	 */
	@Override
	public void postDataToTimeseriesWebsocket(
			DatapointsIngestion datapointsIngestion) {
		String request = this.jsonMapper.toJson(datapointsIngestion);
		log.debug(request);

		try {
			if (this.mListener == null) {
				registerDefaultMessageListener();
			}
			this.wsClient.postTextWSData(request, this.mListener);
	...
	@Override
	public DatapointsResponse queryForDatapoints(String baseUrl,
			DatapointsQuery datapoints, List<Header> headers) {
		...
		try {
			...
			log.debug(request);
			httpResponse = this.restClient.post(baseUrl
					+ TimeSeriesAPIV1.datapointsURI,
			...
			String responseEntity = processHttpResponseEntity(httpResponse
					.getEntity());
			if (responseEntity == null)
				return null;
			response = this.jsonMapper.fromJson(responseEntity,
					DatapointsResponse.class);
			return response;
		} catch (IOException e) {
			throw new RuntimeException("Error occured calling=" + baseUrl //$NON-NLS-1$
	...
	@Override
	public DatapointsResponse queryForLatestDatapoint(String baseUrl,
			DatapointsLatestQuery latestDatapoints, List<Header> headers) {
		DatapointsResponse response = null;
	...
	public TagsList listTags(String baseUrl, List<Header> headers) {
		...
		CloseableHttpResponse httpResponse = null;
		try {
			httpResponse = this.restClient.get(baseUrl
					+ TimeSeriesAPIV1.tagsURI, headers,
					this.timeseriesRestConfig.getTimeseriesConnectionTimeout(),
					this.timeseriesRestConfig.getTimeseriesSocketTimeout());
			handleIfErrorResponse(httpResponse);
			String responseEntity = processHttpResponseEntity(httpResponse
			...
			if (responseEntity == null)
				return null;
			responseTagList = this.jsonMapper.fromJson(responseEntity,
					TagsList.class);
	...
	@Override
	public AggregationsList listAggregations(String baseUrl,
			List<Header> headers) {
		AggregationsList responseAggregationsList = null;
		CloseableHttpResponse httpResponse = null;
		try {
			httpResponse = this.restClient.get(baseUrl
					+ TimeSeriesAPIV1.aggregationsURI, headers,
					this.timeseriesRestConfig.getTimeseriesConnectionTimeout(),
					this.timeseriesRestConfig.getTimeseriesSocketTimeout());
			handleIfErrorResponse(httpResponse);
			String responseEntity = processHttpResponseEntity(httpResponse
			...
			if (responseEntity == null)
				return null;
			responseAggregationsList = this.jsonMapper.fromJson(responseEntity,
					AggregationsList.class);
	...
	private String processHttpResponseEntity(org.apache.http.HttpEntity entity)
		...
		if (entity instanceof GzipDecompressingEntity) {
			return IOUtils.toString(
					((GzipDecompressingEntity) entity).getContent(), "UTF-8"); //$NON-NLS-1$
		}
	...
	private static void handleIfErrorResponse(CloseableHttpResponse httpResponse) {
		if (httpResponse.getStatusLine() == null) {
			...
			throw new RuntimeException(
					"No Status response was received. Locale:" //$NON-NLS-1$
							+ httpResponse.getLocale());
		}
		if (httpResponse.getStatusLine().getStatusCode() >= 300) {
			log.info("Query was unsuccessful. Status Code:" //$NON-NLS-1$
					+ httpResponse.getStatusLine().getStatusCode());
			log.info((httpResponse == null) ? "Response body was empty" //$NON-NLS-1$
